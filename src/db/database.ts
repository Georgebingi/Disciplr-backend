...
export function updateAnalyticsSummary(): void {
    ...
    db.prepare(`UPDATE vault_analytics_summary SET total_vaults = ?, active_vaults = ?, completed_vaults = ?, failed_vaults = ?, total_locked_capital = ?, active_capital = ?, success_rate = ?, last_updated = datetime('now') WHERE id = 1`).run(stats.total_vaults || 0, stats.active_vaults || 0, stats.completed_vaults || 0, stats.failed_vaults || 0, (stats.total_locked_capital || 0).toString(), (stats.active_capital || 0).toString(), successRate)
}
...
export function getTimeRangeFilter(period: string): { startDate: string; endDate: string } {
    ...
    switch (period) {
        case '7d': startDate = new Date(Date.now() - 7 * 24 * 60 * 60 * 1000).toISOString(); break
        case '30d': startDate = new Date(Date.now() - 30 * 24 * 60 * 60 * 1000).toISOString(); break
        case '90d': startDate = new Date(Date.now() - 90 * 24 * 60 * 60 * 1000).toISOString(); break
        case '1y': startDate = new Date(Date.now() - 365 * 24 * 60 * 60 * 1000).toISOString(); break
        default: startDate = new Date(0).toISOString()
    }
